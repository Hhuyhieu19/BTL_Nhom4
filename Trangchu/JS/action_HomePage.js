...
var image = new  Array()
image[0] = '../IMG/Trangchu/Home/laptoptuutruong_800x450.jpg'
image[1] = '../IMG/Trangchu/Home/news_Sale.jpg'
image[2] = '../IMG/Trangchu/Home/laptop-asus-800-450_800x450.png'
...
const updateCountShopping = () => {
    console.log('truy xuất vào localstorage và count item trong này ra để hiển thị số item trong card ra')
    const shopingCart = JSON.parse(localStorage.getItem('ShoppingCart'));
    const countItem = shopingCart ? shopingCart.name.length: 0;
    countItemInCart.innerHTML = countItem;
};
...
function isShowNav() {
    ...
    if (sts.style.display == 'block') return true;
    else return false;
}
...
shownav.addEventListener('click', function () {
    ...
    if (isShowNav()) sts.style.display = 'none';
    else sts.style.display = 'block';
});
...
container_AddCart.addEventListener('click', function (e) {
    ...
    console.log('ShoppingCart: set vào localstorage tại đây.')
